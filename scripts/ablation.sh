...
python3.14 -m washLTH -f="$EVALUATION_DIRECTORY" evaluate --dataset="$DATASET" --baseline="$MODEL" --max-evaluations=15 \
	--mask-filter="magnitude" --sql-filter="WHERE name REGEXP '$RUN_NAME \((Unt|T)rained\) \(\d+\.?\d*%\)'" --ablation;
...
#RUN_NAME="Ablation $NOW";
#EVALUATION_DIRECTORY="$HOME/evaluation/ablation_$NOW";
#time python3.14 -m washLTH -f="$EVALUATION_DIRECTORY" evaluate --dataset="$DATASET" --baseline="$MODEL" --max-evaluations=15 \
#	--mask-filter="magnitude" --sql-filter="WHERE name REGEXP '$RUN_NAME \((Unt|T)rained\) \(\d+\.?\d*%\)'" --ablation;